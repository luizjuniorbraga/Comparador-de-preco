    ...
    try {
        ...
        if (result.via === 'proxy') {
            modeNote.textContent =
                'Arquivo aberto direto — busca via proxy (pode demorar). Para mais estabilidade rode "node server.js" no terminal.'
...
async function searchProducts(query) {
    ...
    throw new Error(
        'Não foi possível buscar no Buscapé. Para funcionamento garantido, rode "node server.js" no terminal e acesse http://localhost:3000.'
    )
}
